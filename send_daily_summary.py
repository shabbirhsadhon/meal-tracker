...
def get_tg_settings(backup):
    return backup.get("months", {}).get("mt3_tg_settings", {})

def get_tg_messages(backup):
    return backup.get("months", {}).get("mt3_tg_messages", [])

# ── Calc helpers ─────────────────────────────────────────────
def get_meal(state, day, user):
    day_data = state.get("meals", {}).get(str(day), state.get("meals", {}).get(day, {}))
    return float(day_data.get(user, 0) or 0)

def get_guest(state, day, user):
    day_data = state.get("guests", {}).get(str(day), state.get("guests", {}).get(day, {}))
    return float(day_data.get(user, 0) or 0)
...
def build_message(backup):
    ...
    # weekday: python Mon=0..Sun=6 → DN রবি=0(Sun)..শনি=6
...
def record_message(backup, msg_text, msg_ids_list):
    """msg_ids_list: [{"chatId": cid, "msgId": mid}, ...]"""
...
def main():
    ...
    if ok_count > 0:
        ...
        try:
            ...
            print("✅ Backup updated on GitHub — message will show in app UI.")
        except Exception as e:
