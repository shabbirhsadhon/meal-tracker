...
def tg_send_one(token, chat_id, text):
    """
    Returns (ok, msg_id, error_code).
    error_code is None on success, an HTTP status int on an HTTP error
    (403/400 usually mean "this chat_id is dead — user blocked the bot,
    deleted their account, or left the chat"), or the string "network" for
    connection-level failures that say nothing about the chat_id itself.
    """
...
def main():
    ...
    for cid in subscribers:
        ok, mid, err = tg_send_one(BOT_TOKEN, cid, msg)
        status  = "✅" if ok else "❌"
